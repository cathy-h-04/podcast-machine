...
def process_pdf_files(files_data):
    ...
    if not isinstance(files_data, list):
        raise ValueError("Expected a list of files, but received " + str(type(files_data)))
    ...
    for i, file_data in enumerate(files_data):
        ...
        if isinstance(file_data, dict) and 'content' in file_data and 'name' in file_data:
            ...
            print(f"File format: dictionary with content and name. Filename: {filename}")
        ...
        try:
            ...
            with os.fdopen(fd, "wb") as tmp:
                try:
                    ...
                    print(f"Successfully decoded and wrote PDF file ({len(pdf_content)} bytes)")
                except Exception as e:
                    print(f"Base64 decoding error: {str(e)}")
                    print(f"Content type: {type(content)}")
                    if isinstance(content, str):
                        print(f"Content length: {len(content)} characters")
                        print(f"Content preview: {content[:30]}..." if len(content) > 30 else content)
